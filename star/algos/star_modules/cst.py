...
class CausalSkillTransformer(nn.Module):
    def __init__(
        self,
        start_token,
        vocab_size,
    ...
    def forward(self, idx, context):
        ...
        x_action = x[:, context.size(1) :, :]
        x_action = self.lnf(x_action)
        logits = self.head(x_action)
        context_summary = self.lnf(x[:, context.size(1) - 1, :])
        return logits, context_summary

    def get_indices_top_k(self, context, codebook_size):
        x = torch.full((context.shape[0], 1), self.start_token, device=context.device, dtype=torch.long)
