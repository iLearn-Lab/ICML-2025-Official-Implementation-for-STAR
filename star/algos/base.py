...
from star.utils.utils import map_tensor_to_device

class Policy(nn.Module, ABC):
    """Common base class for policies used by the training loop."""
    ...
    def obs_encode(self, data, hwc=False, obs_key='obs'):
        ...
        for lowdim_name in self.lowdim_encoders.keys():
            lowdim_encodings.append(self.lowdim_encoders[lowdim_name](data[obs_key][lowdim_name]))
...
class ChunkPolicy(Policy):
    """Base class for policies that predict action chunks."""
    def __init__(self, 
                 action_horizon,
                 **kwargs):
