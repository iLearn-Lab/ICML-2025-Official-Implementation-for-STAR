...
import star.utils.tensor_utils as TU
...
class ObservationKeyToModalityDict(dict):
    ...
    def __getitem__(self, item):
        if item not in self.keys():
            warnings.warn(
                f"Observation key `{item}` was not registered. Falling back to `low_dim`.",
                stacklevel=2,
            )
...
def initialize_obs_utils_with_obs_specs(obs_modality_specs):
    ...
    OBS_MODALITIES_TO_KEYS = {
        obs_modality: sorted(set(obs_modality_mapping[obs_modality]))
